...
- **卡片**：`.card/.services .icon-box/.team .member/.explore-card/.hl-card/.hl-feature` 同一套表面（白底、`--c-line` 邊、`--r-lg`、`--shadow-1`，hover 上浮 4px＋`--shadow-2`＋金色邊）。首頁「代表成果」（2026-09-25 取代精選歷程清單）＝四張 1:1 圖卡（`.hl-card`，手機 4:3）＋創作子區兩張等寬 16:9 大圖卡（`.hl-feature`，左上深色藥丸標 AI 生成）；書封卡用三張真實封面疊扇（`.hl-media--trio`）。
