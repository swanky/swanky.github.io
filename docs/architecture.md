...
- **Card art**: AI-generated 墨線復古 uniform-girl deck (all 78 rendered PNGs). All tarot imagery lives under one tree `assets/img/tarot/`: `uniform/` + `clonex/` (768×1152 web JPGs), `rws/` (public-domain Waite 720px comparison set), and `print/` (in git but excluded from `_site` via `_config.yml`) holding `uniform/`+`clonex/` 1024×1536 masters and `rws/` 1080×1920 scans. Pip counts must be verified by high-zoom crops of the full-res PNG — low-zoom full-frame counting undercounts.
