...
1. 確認輸入指定的檔案存在（使用目前環境的原生 shell 執行命令，如 `test -f`）。
...
使用目前環境的原生 shell 執行 Python 腳本。根據檔案類型選擇對應策略。**統一使用 `python -` 搭配 heredoc 執行**。
...
**自適應縮放邏輯**：以 2560px 為目標寬度動態計算 scale（下限 2.0），取代固定 4x。標準投影片（720pt 寬）產生 ~2560×1920 圖片，A4 頁面（595pt 寬）產生 ~2560×3620 圖片。圖檔大小較固定 4x 減少 30–50%，加速後續讀取與檢查。

#### 情況 B：PPTX / PPT 檔案

依序嘗試 B1 → B2 → B3。

**B1 — Windows PowerPoint COM（需安裝 MS Office；Windows＋Python 環境下適用）**
...
**B2 — LibreOffice headless → PDF → PNG（跨平台 fallback，需已安裝 LibreOffice）**
...
轉換完成後，列出輸出目錄下所有 `page_*.png`（依頁碼排序）。若 `_text_hints.json` 存在，先讀取並檢查其內容作為解讀輔助。

#### 策略選擇

| 頁數 | 策略 |
|------|------|
| ≤ 10 | **循序處理**：依序讀取每頁圖片，當場解讀 |
| 11–50 | **平行處理**：若 agent 支援 delegation 且任務可獨立切分，分批同時解讀；否則依序處理 |
| > 50 | 先詢問使用者是否分批或指定頁碼範圍 |

#### 平行處理細節（頁數 > 10 且 agent 支援 delegation 時啟用）

將頁面分為多個批次（每批 5–8 頁），對每批委派一個獨立子任務同時處理。**同一批次的所有委派應同時發出以實現真正平行。**
...
收集所有子任務回傳後，依頁碼排序合併。若 agent 不支援 delegation，則依序自行處理全部頁面。
...
將完整 Markdown 寫入 `<safe_name>_slides.md`。
...
| 委派的子任務回傳缺漏頁面 | 對缺漏頁面 fallback 循序處理 |
