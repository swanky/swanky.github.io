...
argument-hint: <linkedin-url 和/或 x-url，順序不拘>
allowed-tools: WebFetch, Bash, Write, Read, Glob, Edit, mcp__playwright__browser_run_code_unsafe, mcp__playwright__browser_navigate
---
...
**⚠️ WebFetch 對 LinkedIn 只會回傳 AI 改寫摘要，不可用於正文擷取。必須用 Playwright 抓逐字原文。**

用 `mcp__playwright__browser_run_code_unsafe` 一次抓齊：
...
X 有三種內容型態，抓法不同。先用 syndication API 判斷型態：
...
**死路備忘（別再浪費時間）**：
- 未登入 Playwright 開 x.com 貼文頁只能看到 Article 預覽卡片；開 `/i/article/...` 直接被重導到登入頁。
- gstack browse 的 `cookie-import-browser` 在 Windows 上對 Chrome/Brave/Edge 一律 `DPAPI decryption failed`（Chromium App-Bound Encryption），不可行。
...
4. **封面**：兩邊封面不同時，選資訊密度高、與內文對應完整的那張（下載兩張用 Read 看過再決定）；相同就選解析度高的。
5. **日期**：取兩者較早的發布日期。
...
- 用 Glob 檢查 `_posts/*<slug>*` 不重複；若已存在，詢問使用者是否覆蓋。
...
1. **盤點現有系列 hub**（用 Glob `technical/*/index.html` 找出 front matter 含 `_groups:` 的頁面）。目前有兩個：
...
2. 用 Read 讀這些 hub 的 front matter，理解每個系列與各分組的 heading／intro／現有文章清單。
...
4. **若判斷相關 →（不要直接改）先向使用者提出建議並等待確認**：
   ...
   - 等使用者確認或調整後，才用 Edit 在該 hub 頁面對應分組的 `posts:` 清單指定位置插入新文章 slug。slug = 文章檔名去掉日期前綴與 `.md`，且須與其他 slug 互不為子字串（避免 `url contains slug` 誤匹配）。
...
再提交（git 操作優先用 Bash 工具，避免 PowerShell 引號亂象）：
...
- **不重複建立**：執行前先用 Glob 搜尋 `_posts/*<slug>*`；同一篇文的第二個來源後補時，是**更新既有檔案**（補 `source_url_x` 或 `source_url`、必要時融合內文），不是開新檔。
- **繁體中文**：所有生成內容（描述、keywords 等）維持繁體中文（zh-TW）。
- **RWA 術語**：若內文涉及 RWA，一律寫「現實世界資產」（不寫真實世界資產）。
- **Edit 假成功**：關鍵改動後用 `git status` / `git diff --stat` 核實檔案真的變了再往下走。
